...
function extractPubId(args) {
    const idx = args.indexOf("--pub-id");
    if (idx === -1)
        return { pubId: undefined, rest: args };
    const pubId = args[idx + 1];
    if (!pubId)
        throw new Error("--pub-id requires a value");
...
async function main() {
    ...
    if (!pubId && !PUB_ID_OPTIONAL.has(command)) {
        throw new Error(`Publication ID required. Set BEEHIIV_PUBLICATION_ID in .env or pass --pub-id <id>.`);
    }
